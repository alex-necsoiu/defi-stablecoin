...
function testRevertsIfTransferFromFails() public {
    // Arrange - Setup
    // Pass 'user' as the owner, since only 'user' can transfer ownership
    MockFailedTransferFrom mockDsc = new MockFailedTransferFrom(user);
    tokenAddresses = [address(mockDsc)];
    feedAddresses = [ethUsdPriceFeed];

    // Deploy DSCEngine (any address can deploy)
    DSCEngine mockDsce = new DSCEngine(tokenAddresses, feedAddresses, address(mockDsc));
    mockDsc.mint(user, amountCollateral);

    // Transfer ownership as the correct owner (user)
    vm.prank(user);
    ...
    // Expect OpenZeppelin's SafeERC20FailedOperation error
    vm.expectRevert(
        abi.encodeWithSelector(SafeERC20.SafeERC20FailedOperation.selector, address(mockDsc))
    );
    ...
    function testMustImproveHealthFactorOnLiquidation() public {
        // Arrange - Setup a mock DSC token that allows more debt than normal
        // Pass 'user' as the owner, since that's who will call transferOwnership
        MockMoreDebtDSC mockDsc = new MockMoreDebtDSC(user, ethUsdPriceFeed);
        tokenAddresses = [weth];
        feedAddresses = [ethUsdPriceFeed];
        address owner = user; // The owner is 'user' (address(1))

        // Log tokenAddresses and feedAddresses
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            console.log("tokenAddresses[%s]: %s", i, tokenAddresses[i]);
        }
        for (uint256 i = 0; i < feedAddresses.length; i++) {
            console.log("feedAddresses[%s]: %s", i, feedAddresses[i]);
        }
        console.log("owner: %s", owner);

        // Deploy DSCEngine as any address (doesn't need to be owner of mockDsc)
        DSCEngine mockDsce = new DSCEngine(tokenAddresses, feedAddresses, address(mockDsc));
        console.log("mockDsce address: %s", address(mockDsce));
        
        // Transfer ownership as the correct owner (user)
        vm.prank(owner);
        mockDsc.transferOwnership(address(mockDsce));
        console.log("transfer Ownership: %s", address(mockDsce));
        ...
        // Simulate a price drop to make the user's position undercollateralized
        int256 ethUsdUpdatedPrice = 18e8; // 1 ETH = $18
        MockV3Aggregator(ethUsdPriceFeed).updateAnswer(ethUsdUpdatedPrice);

        // Act/Assert - Expect revert if health factor is not improved after liquidation
